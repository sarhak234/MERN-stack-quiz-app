        ...
        {data ? (
          <form onSubmit={(e) => e.preventDefault()} className="space-y-6 sm:space-y-8">
            {data.questions.map((q, index) => (
              ...
              >
                <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-900 mb-3 sm:mb-4 flex items-start gap-2 sm:gap-3">
                  <span className="bg-blue-100 text-blue-700 rounded-full w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 flex items-center justify-center text-xs sm:text-sm md:text-base shrink-0">
                    {index + 1}
                  </span>
                  <span className="flex-1 pl-6 sm:pl-7 md:pl-8">
                    {q.question}
                  </span>
                </h3>
